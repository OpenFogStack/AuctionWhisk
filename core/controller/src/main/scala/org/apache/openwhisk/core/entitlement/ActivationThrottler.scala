 ...
 */
class ActivationThrottler(loadBalancer: LoadBalancer, concurrencyLimit: Identity => Int, invokationsPerTimeFrame: Long, timeFrame: Long)(
  ...
  def isOverloaded()(implicit tid: TransactionId): Future[Boolean] = {
    println(s"in isOverloaded method the current activations count is: ${loadBalancer.totalActivationsDuringTimeFrame}")
    loadBalancer.totalActivationsDuringTimeFrame.accumulate(1)
    val overloaded: Boolean = invokationsPerTimeFrame < loadBalancer.totalActivationsDuringTimeFrame.get()
    if (overloaded)
      logging.info(
        this,
        s"concurrent activations in system = ${loadBalancer.totalActivationsDuringTimeFrame}, below limit = ${loadBalancer.totalActivationsDuringTimeFrame}")
    Future(overloaded)
  }
}
