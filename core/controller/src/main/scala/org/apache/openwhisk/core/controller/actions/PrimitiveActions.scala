...
protected[actions] trait PrimitiveActions {
  ...
  private def invokeSimpleAction(
    ...
    val postedFuture : Future[Future[Either[ActivationId, WhiskActivation]]] = loadBalancer.publish(action, message)

    postedFuture andThen {
      case Success(_) => transid.finished(this, startLoadbalancer)
      case Failure(e) => transid.failed(this, startLoadbalancer)
      ...
      waitForResponse
        .map { timeout =>
          if(activeAckResponse.isCompleted) {
            Future.successful(Left(message.activationId))
          } else {
            logging.info(this, "I shouldn't be here. Future is completed:" + activeAckResponse.isCompleted)
