...
import org.apache.openwhisk.core.entity.{BulkEntityResult, DocInfo, DocInfoAbstraction, DocumentReader, EntityName, EntityPath, ExecMetaDataBase, NodeStorageLimit, UUID, WhiskActionMetaData}
import org.apache.openwhisk.http.Messages
import pureconfig._
import org.apache.openwhisk.core.ConfigKeys
import org.apache.openwhisk.core.entity.MyJsonProtocol._
...
class CouchDbRestStore[DocumentAbstraction <: DocumentSerializer](dbProtocol: String,
  ...
  private val config_size = NodeStorageLimit.toBytes(loadConfigOrThrow[String](ConfigKeys.nodeStorageLimit))
  ...
  override protected[database] def put(d: DocumentAbstraction)(implicit transid: TransactionId): Future[DocInfo] = {
    ...
    val entityType = asJson.fields("entityType").toString()

    if(entityType.equals("\"action\"")) {
      val bids = asJson.fields("bids").convertTo[List[StorageBidLimit]]
      val result = Await.result(getDBStats(), 2.seconds)

      if((BigDecimal(config_size) * 0.9 ) < BigDecimal(result)) {

        val combined = for (
          n1 <- findWithLowerStorageBid(bids.filter(x => x.nodeId.equals(nodeId)).head.bidStorage);
          n2 <- this.del(n1)
        ) yield n2
        Await.result(combined, 10.seconds)
      }
  ...
  protected[database] def findWithLowerStorageBid(bid: Double)(implicit transid: TransactionId): Future[DocInfo]= {

    val start = transid.started(this, LoggingMarkers.DATABASE_QUERY, s"[POST] '$dbName' find with lower bid")

    val f = client.findArraydWithLowerStorageBid(bid, nodeId).map { e =>
      e match {
        case Right(response) =>

          val list = response.fields("docs").convertTo[List[DocInfoAbstraction]]

          if(list.size<1) {
            throw DatabaseFullException("The database is full. Use another node.") // TODO ersetzen durch 4xx Exception
          }
          val lowestBidObject = list.map(i => i.bids).flatten.filter(x => x.nodeId.equals(nodeId)).minBy(_.bidStorage)
          val lowestEntry = list.filter(x => x.bids.contains(lowestBidObject)).head.toDocInfo()

          transid.finished(this, start, s"[POST] '$dbName' completed finding lower bid: , response: $response")
          lowestEntry
        case Left(StatusCodes.NotFound) =>
          transid.finished(this, start, s"[POST] '$dbName', lower bid: not found.")
          // for compatibility
          throw NoDocumentException("not found on 'delete'")
        ...
        case Left(code) =>
          transid.failed(
            this,
            start,
            s"[POST] '$dbName' failed to get lower bid; http status: '${code}'",
            ErrorLevel)
          throw new DeleteException("Unexpected http response code: " + code)
      }
    }
    ...
    reportFailure(
      ...
      failure =>
        transid.failed(
          this,
          start,
          s"[POST] '$dbName' internal error finding lower bid, doc, failure: '${failure.getMessage}'",
          ErrorLevel))
  }

  protected[database] def getDBStats()(implicit transid: TransactionId): Future[BigInt] = {

    val start = transid.started(this, LoggingMarkers.DATABASE_GET, s"[GET] '$dbName' getting stats")

    val f = client.getDBStats().map { e =>
      e match {
        case Right(response) =>
          transid.finished(this, start, s"[GET] '$dbName' completed stats query: , response: $response")
          response.fields("data_size").convertTo[BigInt]

        case Left(StatusCodes.NotFound) =>
          transid.finished(this, start, s"[GET] '$dbName', db stats: not found.")
          // for compatibility
          throw NoDocumentException("not found on 'delete'")

        case Left(StatusCodes.Conflict) =>
          transid.finished(this, start, s"[GET] '$dbName', document: stats conflict.")
          throw DocumentConflictException("conflict on 'delete'")
        ...
        case Left(code) =>
          transid.failed(
            this,
            start,
            s"[GET] '$dbName' failed to get stats; http status: '${code}'",
            ErrorLevel)
          throw new DeleteException("Unexpected http response code: " + code)
      }
    }
    ...
    reportFailure(
      ...
      failure =>
        transid.failed(
          this,
          start,
          s"[GET] '$dbName' internal error while loading stats, doc, failure: '${failure.getMessage}'",
          ErrorLevel))
  }

  protected[database] def createIndexOnStorageBid()(implicit transid: TransactionId): Future[String] = {

    val start = transid.started(this, LoggingMarkers.DATABASE_GET, s"[POST] '$dbName' create index on storage bid")

    val f = client.createIndexOnStorageBid().map { e =>
      e match {
        case Right(response) =>

          transid.finished(this, start, s"[POST] '$dbName' create index finished: , response: $response")
          response.fields("result").convertTo[String]
          "test"
        case Left(StatusCodes.NotFound) =>
          transid.finished(this, start, s"[GET] '$dbName', create index: not found.")
          // for compatibility
          throw NoDocumentException("not found on 'delete'")
        ...
        case Left(code) =>
          transid.failed(
            this,
            start,
            s"[POST] '$dbName' failed to set index; http status: '${code}'",
            ErrorLevel)
          throw new DeleteException("Unexpected http response code: " + code)
      }
    }
    ...
    val f = request(client).map { e =>
      e match {
        ...
        case Left(StatusCodes.NotFound) =>
          ...
          //throw NoDocumentException("not found on 'get'") // TODO return this action and if else switch
          if(ma.runtimeClass == classOf[WhiskActionMetaData]) {

            val xyz = """{"_id":"guest/hello14","_rev":"1-c12346f74e549fd152454bfa469efcab","annotations":[{"key":"provide-api-key","value":false},{"key":"exec","value":"nodejs:10"}],"bids":[{"bidExecution":55.0,"bidStorage":55.0,"nodeId":"116.203.137.81"},{"bidExecution":55.0,"bidStorage":55.0,"nodeId":"116.203.214.251"}],"entityType":"action","exec":{"binary":false,"code":{"attachmentName":"mem:","attachmentType":"text/plain; charset=UTF-8","digest":"sha256-bfdb78a044954e399587c7678693142855dcaa5f867702f2ee318f8438983922","length":387},"kind":"nodejs:10"},"limits":{"concurrency":1,"logs":10,"memory":256,"timeout":60000},"name":"toMockAction","namespace":"guest","parameters":[],"publish":false,"updated":1591024930810,"version":"0.0.1"}"""
            deserialize[A, DocumentAbstraction](doc, xyz.parseJson.asJsObject)
            } else {
  ...
  def createWhiskActionMetaDataMock: Future[WhiskActionMetaData] = {

    val exec = "{  \"kind\": \"nodejs:10\",        \"code\": \"function main(params) { var array = [], upperLimit = Math.sqrt(params.number), output = []; for (var i = 0; i < params.number; i++) { array.push(true); } for (var i = 2; i <= upperLimit; i++) { if (array[i]) { for (var j = i * i; j < params.number; j += i) { array[j] = false; } } } for (var i = 2; i < params.number; i++) { if(array[i]) { output.push(i); } } return {payload: output}; }\",        \"binary\": false    }"



    Future(new WhiskActionMetaData(namespace = EntityPath("guest"), name = EntityName("waslife"), ExecMetaDataBase(exec)))
  }
