...
import org.apache.openwhisk.core.entity.{CacheKey, DocId, DocInfo, DocRevision}
...
trait DocumentFactory[W <: DocumentRevisionProvider] extends MultipleReadersSingleWriterCache[W, DocInfo] {
  ...
  def setIndex[Wsuper >: W](db: ArtifactStore[Wsuper])(
    implicit transid: TransactionId,
    notifier: Option[CacheChangeNotification]): Future[String] = {
    implicit val logger = db.logging
    implicit val ec = db.executionContext

    db.createIndexOnStorageBid()
  }
  ...
  def setIndexonStorageBid[Wsuper >: W](db: ArtifactStore[Wsuper])(implicit transid: TransactionId): Unit = {
      db.createIndexOnStorageBid()
  }
}
