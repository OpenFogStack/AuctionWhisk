...
trait WhiskActionsApi extends WhiskCollectionAPI with PostActionActivation with ReferencedEntities {
        ...
        entity(as[Option[JsObject]]) { payload =>
          getEntity(WhiskActionMetaData.resolveActionAndMergeParameters(entityStore, entityName), Some {
            act: WhiskActionMetaData =>
              println(act.name)
              logging.info(this, "completed resolveActionAndMergeParameters method")
              ...
              onComplete(entitleReferencedEntitiesMetaData(user, Privilege.ACTIVATE, Some(action.exec))) {
                case Success(_) =>
                  ...
                  if(allowInvoke && action.name.name.equals("toMockAction")){
                    escalateToNextNode(entityName.name.name)
                  } else if (allowInvoke) {
                ...
                case Failure(f) =>
                  logging.info(this, "this is a test log")
                  super.handleEntitlementFailure(f)
              }
          })
        }
  ...
  private def escalateToNextNode(actionName: String)(
    implicit transid: TransactionId): RequestContext => Future[RouteResult] = {

    logging.info(this, "escalate to next level node because no storage has been found.")

    val result = Http("https://"+ nextNode +"/api/v1/namespaces/_/actions/" + actionName +"?blocking=true").option(HttpOptions.allowUnsafeSSL).postData("""{"number":100}""")
    ...
    logging.info(this, "received response from upper node")
    complete(OK, ActivationId("02bc0353a3894badbc0353a3896bad74").toJson) // evtl. die activID ersetzen durch richtige response
  }
  ...
  private def makeWhiskAction(content: WhiskActionPut, entityName: FullyQualifiedEntityName)(
    implicit transid: TransactionId) = {
    logging.info(this, "test hallo" + content)
    logging.info(this,"Hallo Hallo -" + content)
    ...
    val bids = content.bids map { l => l
    } getOrElse List(StorageBidLimit())
  ...
  private def updateWhiskAction(content: WhiskActionPut, action: WhiskAction)(implicit transid: TransactionId) = {
    ...
    val bids = content.bids map { l =>
        List(StorageBidLimit())

    } getOrElse action.bids
