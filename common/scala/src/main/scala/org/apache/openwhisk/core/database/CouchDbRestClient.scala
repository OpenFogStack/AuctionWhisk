...
class CouchDbRestClient(protocol: String, host: String, port: Int, username: String, password: String, db: String)(
  ...
  // selfmade, return the db stats to retrieve storage
  def getDBStats(): Future[Either[StatusCode, JsObject]] = {
    requestJson[JsObject](mkRequest(HttpMethods.GET, uri(db), headers = baseHeaders))
  }

  //selfmade
  def findWithLowerStorageBid(bid: Double): Future[Either[StatusCode, JsObject]] =
    requestJson[JsObject](
      mkJsonRequest(HttpMethods.POST, uri(db, "_find"), JsObject("selector" -> JsObject("bids.bidStorage" -> JsObject("$lt" -> JsNumber(bid))),
        "fields" -> JsArray(Vector(JsString("_id"), JsString("_rev") )), "limit" -> JsNumber(1), "sort" -> JsArray(Vector(JsObject("bids.bidStorage" -> JsString("asc"))))), baseHeaders))

  def findArraydWithLowerStorageBid(bid: Double, nodeId: String): Future[Either[StatusCode, JsObject]] = {

    val bidStorage = JsObject("$lt" -> JsNumber(bid))
    val nodeIdJson =  JsObject("$eq" -> JsString(nodeId))
    val elemMatch = JsObject( "$elemMatch" -> JsObject("bidStorage" -> bidStorage, "nodeId" -> nodeIdJson ))
    val selector = "selector" -> JsObject("bids" -> elemMatch )
    val field = "fields" -> JsArray(Vector(JsString("_id"), JsString("_rev"), JsString("bids") ))

    println(JsObject(selector, field).prettyPrint)

  requestJson[JsObject](
      mkJsonRequest(HttpMethods.POST, uri(db, "_find"), JsObject(selector, field), baseHeaders))
  }


  // not used
  def createIndexOnStorageBid(): Future[Either[StatusCode, JsObject]] = {
  requestJson[JsObject](
    mkJsonRequest(HttpMethods.POST, uri(db, "_index"), JsObject("index" ->
      JsObject("fields" -> JsArray(Vector(JsString("bids.bidStorage"))))), baseHeaders))
  }
