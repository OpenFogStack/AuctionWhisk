  ...
  it should "put and then get an action from cache" in {
    ...
    actions.foreach {
      case (action, kind) =>
        ...
        println("-----")
        println(s"$collectionPath/${action.name}")
        println(content.toJson)
        println(creds)
        println("-----")
      ...
      case (action, kind) =>
        val content = WhiskActionPut(
          ...
          Some(
            ...
            Some(
              ActionBidsOption(
                Some(action.bids.bidExecutionCloud),
                Some(action.bids.bidExecutionFog),
                Some(action.bids.bidExecutionEdge),
                Some(action.bids.bidStorageCloud),
                Some(action.bids.bidStorageFog),
                Some(action.bids.bidStorageEdge))))
