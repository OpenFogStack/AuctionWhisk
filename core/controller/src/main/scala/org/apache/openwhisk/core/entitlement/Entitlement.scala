...
protected[core] abstract class EntitlementProvider(
  ...
  private val concurrentInvokeThrottler =
    new ActivationThrottler(
      ...
      loadConfigOrThrow[Long](ConfigKeys.actionInvokesPerTimeFrame),loadConfigOrThrow[Long](ConfigKeys.actionInvokesTimeFrame) )
  ...
  protected def checkSystemOverload(right: Privilege)(implicit transid: TransactionId): Future[Unit] = {
    concurrentInvokeThrottler.isOverloaded.flatMap { isOverloaded => // TODO check überarbeiten
      if (isOverloaded) {
        logging.error(this, "system is overloaded")
        Future.failed(RejectRequest(TooManyRequests, "our service experiences too many request. Forward to the next node on the way to the cloud."))
      } else Future.successful(())
    }
  }
