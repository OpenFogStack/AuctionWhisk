...
case class ActionBidsOption(bids: Option[List[StorageBidLimit]])
...
case class WhiskActionPut(exec: Option[Exec] = None,
                          ...
                          bids: Option[List[StorageBidLimit]] = None,
                          version: Option[SemVer] = None,
                          publish: Option[Boolean] = None,
                          annotations: Option[Parameters] = None) {

  protected[core] def replace(exec: Exec) = {
    WhiskActionPut(Some(exec), parameters, limits, bids, version, publish, annotations)
  }
  ...
  protected[core] def resolve(userNamespace: Namespace): WhiskActionPut = {
    exec map {
      case SequenceExec(components) =>
        ...
        WhiskActionPut(Some(newExec), parameters, limits, bids, version, publish, annotations)
      case _ => this
    } getOrElse this
  }
}
...
case class WhiskAction(namespace: EntityPath,
                       ...
                       bids: List[StorageBidLimit] = List(StorageBidLimit()),
  ...
  def toExecutableWhiskAction: Option[ExecutableWhiskAction] = exec match {
    case codeExec: CodeExec[_] =>
      Some(
        ExecutableWhiskAction(namespace, name, codeExec, parameters, limits,bids, version, publish, annotations)
...
case class ExecutableWhiskAction(namespace: EntityPath,
  ...
  require(exec != null, "exec undefined")
  require(limits != null, "limits undefined")
  require(bids != null, "bids undefined")
  /**
  ...
  def toWhiskAction =
    WhiskAction(namespace, name, exec, parameters, limits, bids, version, publish, annotations)
...
case class ExecutableWhiskActionMetaData(namespace: EntityPath,
  ...
  def toWhiskAction =
    WhiskActionMetaData(namespace, name, exec, parameters, limits, bids, version, publish, annotations, updated)
  ...
  def resolveActionAndMergeParameters(entityStore: EntityStore, fullyQualifiedName: FullyQualifiedEntityName)(
    ...
    if (entityPath.defaultPackage) {
      ...
      wp flatMap { resolvedPkg =>
        ...
        val fqnAction = resolvedPkg.fullyQualifiedName(withVersion = false).add(actionName)
        // get the whisk action associate with it and inherit the parameters from the package/binding
        WhiskActionMetaData.get(entityStore, fqnAction.toDocId) map { x =>
          println("second whisk action inherit")
          x.inherit(
...
object ActionBidsOption extends DefaultJsonProtocol {
  implicit val serdes = jsonFormat1(ActionBidsOption.apply)
}
