...
trait LoadBalancer {
  ...
  // selfmade var
  val totalActivationsDuringTimeFrame: LongAccumulator = new LongAccumulator((x,y) => x+y, 0)

  val actionInvokesTimeFrame =  loadConfigOrThrow[Long](ConfigKeys.actionInvokesTimeFrame)

  val allowedActionInvokesPerTimeFrame = loadConfigOrThrow[Long](ConfigKeys.actionInvokesPerTimeFrame)

  val lastMinuteActivationCounter: LongAccumulator = new LongAccumulator((x,y) => x+y, 0)

  val lastMinuteExecBids: LongAccumulator = new LongAccumulator((x,y) => x+y, 0)

  val lastMinuteAVGExecBid: LongAccumulator = new LongAccumulator((x,y) => x+y, 0)
